...
`199 agent-format traces` distilled from **hf_magicoder_django_filter** via
...
| Accepted (score >= 84.8) | 199 (66.3%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-04-28T18:51:36 |
...
- **Source dataset**: hf_magicoder_django_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_django")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_django/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_django/data.jsonl \
...
- Acceptance rate 66.3% означает что 33.7% raw
...
MIT License (inherited from source dataset hf_magicoder_django_filter)

This subset is a derivative work distilled from hf_magicoder_django_filter (MIT-licensed)
