# distilled_kubernetes

`79 agent-format traces` distilled from **hf_magicoder_kubernetes_filter** via
...
| Metric | Value |
|---|---|
| Raw source items | 200 |
| Accepted (score >= 84.8) | 79 (39.5%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-05-01T14:21:10 |

## Why this subset exists

Этот subset был distilled для расширения покрытия в категории kubernetes.
...
- **Source dataset**: hf_magicoder_kubernetes_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_kubernetes")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_kubernetes/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_kubernetes/data.jsonl \
...
- Acceptance rate 39.5% означает что 60.5% raw
...
MIT License (inherited from source dataset hf_magicoder_kubernetes_filter)

This subset is a derivative work distilled from hf_magicoder_kubernetes_filter (MIT-licensed)
