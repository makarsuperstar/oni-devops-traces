# distilled_ci_cd_specific

`215 agent-format traces` distilled from **hf_magicoder_ci_cd_specific_filter** via
...
| Accepted (score >= 84.8) | 215 (86.0%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-05-01T20:30:24 |

## Why this subset exists

Этот subset был distilled для расширения покрытия в категории ci_cd_specific.
...
- **Source dataset**: hf_magicoder_ci_cd_specific_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_ci_cd_specific")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_ci_cd_specific/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_ci_cd_specific/data.jsonl \
...
- Acceptance rate 86.0% означает что 14.0% raw
...
MIT License (inherited from source dataset hf_magicoder_ci_cd_specific_filter)

This subset is a derivative work distilled from hf_magicoder_ci_cd_specific_filter (MIT-licensed)
