# distilled_bash_pipes

`243 agent-format traces` distilled from **hf_magicoder_bash_pipes_filter** via
...
| Accepted (score >= 84.8) | 243 (81.0%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-04-28T09:14:35 |
...
- **Source dataset**: hf_magicoder_bash_pipes_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_bash_pipes")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_bash_pipes/data.jsonl
```
...
- Acceptance rate 81.0% означает что 19.0% raw
...
MIT License (inherited from source dataset hf_magicoder_bash_pipes_filter)

This subset is a derivative work distilled from hf_magicoder_bash_pipes_filter (MIT-licensed)
