...
`199 agent-format traces` distilled from **hf_magicoder_express_filter** via
...
| Accepted (score >= 84.8) | 199 (79.6%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-04-29T02:41:45 |
...
- **Source dataset**: hf_magicoder_express_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_express")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_express/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_express/data.jsonl \
...
- Acceptance rate 79.6% означает что 20.4% raw
...
MIT License (inherited from source dataset hf_magicoder_express_filter)

This subset is a derivative work distilled from hf_magicoder_express_filter (MIT-licensed)
