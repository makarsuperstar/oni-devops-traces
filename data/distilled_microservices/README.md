# distilled_microservices

`133 agent-format traces` distilled from **hf_magicoder_microservices_filter** via
...
| Accepted (score >= 84.8) | 133 (53.2%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-04-29T09:07:07 |
...
- **Source dataset**: hf_magicoder_microservices_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_microservices")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_microservices/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_microservices/data.jsonl \
...
- Acceptance rate 53.2% означает что 46.8% raw
...
MIT License (inherited from source dataset hf_magicoder_microservices_filter)

This subset is a derivative work distilled from hf_magicoder_microservices_filter (MIT-licensed)
