...
`8 agent-format traces` distilled from **hf_magicoder_eslint_filter** via
...
|---|---|
| Raw source items | 10 |
| Accepted (score >= 84.8) | 8 (80.0%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-05-03T12:38:19 |

## Why this subset exists

Этот subset был distilled для расширения покрытия в категории eslint.
...
- **Source dataset**: hf_magicoder_eslint_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_eslint")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_eslint/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_eslint/data.jsonl \
...
- Acceptance rate 80.0% означает что 20.0% raw
...
MIT License (inherited from source dataset hf_magicoder_eslint_filter)

This subset is a derivative work distilled from hf_magicoder_eslint_filter (MIT-licensed)
