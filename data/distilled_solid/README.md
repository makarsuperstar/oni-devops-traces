...
`192 agent-format traces` distilled from **hf_magicoder_solid_filter** via
...
| Accepted (score >= 84.8) | 192 (76.8%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-04-29T22:58:41 |
...
- **Source dataset**: hf_magicoder_solid_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_solid")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_solid/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_solid/data.jsonl \
...
- Acceptance rate 76.8% означает что 23.2% raw
...
MIT License (inherited from source dataset hf_magicoder_solid_filter)

This subset is a derivative work distilled from hf_magicoder_solid_filter (MIT-licensed)
