# distilled_postgres_advanced

`150 agent-format traces` distilled from **hf_magicoder_postgres_advanced_filter** via
...
| Accepted (score >= 84.8) | 150 (60.0%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-05-02T02:27:10 |

## Why this subset exists

Этот subset был distilled для расширения покрытия в категории postgres_advanced.
...
- **Source dataset**: hf_magicoder_postgres_advanced_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_postgres_advanced")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_postgres_advanced/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_postgres_advanced/data.jsonl \
...
- Acceptance rate 60.0% означает что 40.0% raw
...
MIT License (inherited from source dataset hf_magicoder_postgres_advanced_filter)

This subset is a derivative work distilled from hf_magicoder_postgres_advanced_filter (MIT-licensed)
