# distilled_docker_advanced

`248 agent-format traces` distilled from **hf_magicoder_docker_advanced_filter** via
...
| Metric | Value |
|---|---|
| Raw source items | 300 |
| Accepted (score >= 84.8) | 248 (82.7%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-04-30T15:01:08 |

## Why this subset exists

Этот subset был distilled для расширения покрытия в категории docker_advanced.
...
- **Source dataset**: hf_magicoder_docker_advanced_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_docker_advanced")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_docker_advanced/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_docker_advanced/data.jsonl \
...
- Acceptance rate 82.7% означает что 17.3% raw
...
MIT License (inherited from source dataset hf_magicoder_docker_advanced_filter)

This subset is a derivative work distilled from hf_magicoder_docker_advanced_filter (MIT-licensed)
