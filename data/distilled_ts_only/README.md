...
`308 agent-format traces` distilled from **hf_magicoder_ts_only_filter** via
...
| Accepted (score >= 84.8) | 308 (77.0%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-05-03T11:35:52 |
...
- **Source dataset**: hf_magicoder_ts_only_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_ts_only")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_ts_only/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_ts_only/data.jsonl \
...
- Acceptance rate 77.0% означает что 23.0% raw
...
MIT License (inherited from source dataset hf_magicoder_ts_only_filter)

This subset is a derivative work distilled from hf_magicoder_ts_only_filter (MIT-licensed)
