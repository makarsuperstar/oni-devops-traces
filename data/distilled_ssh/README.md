...
`225 agent-format traces` distilled from **hf_magicoder_ssh_filter** via
...
| Accepted (score >= 84.8) | 225 (75.0%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-04-30T06:39:02 |

## Why this subset exists

Этот subset был distilled для расширения покрытия в категории ssh.
...
- **Source dataset**: hf_magicoder_ssh_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_ssh")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_ssh/data.jsonl
```
...
- Acceptance rate 75.0% означает что 25.0% raw
...
MIT License (inherited from source dataset hf_magicoder_ssh_filter)

This subset is a derivative work distilled from hf_magicoder_ssh_filter (MIT-licensed)
