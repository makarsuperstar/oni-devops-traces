...
`351 agent-format traces` distilled from **hf_magicoder_js_only_filter** via
...
| Accepted (score >= 84.8) | 351 (87.7%) |
| Format compliance | 100% (filter threshold) |
| Distilled at | 2026-05-03T00:17:10 |
...
- **Source dataset**: hf_magicoder_js_only_filter
...
ds = load_dataset("path/to/repo", data_dir="data/distilled_js_only")
```

### As raw JSONL

```bash
wget https://raw.githubusercontent.com/<USER>/oni-devops-traces/main/data/distilled_js_only/data.jsonl
```
...
cat data/own_anchors/train.jsonl \
    data/distilled_js_only/data.jsonl \
...
- Acceptance rate 87.7% означает что 12.3% raw
...
MIT License (inherited from source dataset hf_magicoder_js_only_filter)

This subset is a derivative work distilled from hf_magicoder_js_only_filter (MIT-licensed)
